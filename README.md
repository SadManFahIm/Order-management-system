...
- Versioned migration runner (`npm run db:migrate` / `db:migrate:down` / `db:migrate:status`) with migrations 001–010; dialect-selectable DB config (`DB_DIALECT` / `DATABASE_URL`, default SQLite); PostgreSQL 16 service in `docker-compose.yml`; migrations run at boot on both dialects
...
│   │   └── __tests__/        # 34 suites · 319 tests
│   ├── migrations/           # Versioned schema migrations (001–010)
...
```bash
cd backend
npm test                      # Vitest — 319 tests across 34 suites (2 skipped locally)
