...
| **Phase 8 follow-ups** — Multi-outlet & franchise management (migration 028) | **Multi-outlet schema** (`outlets`, `outlet_memberships`, `outlet_menu_overrides` tables — physical branches within a tenant; each outlet carries name, code, slug, timezone, status, opening hours and settings), **outlet_id FK columns** on 10 operational tables (`orders`, `inventory_items`, `tables`, `tenant_closure_dates`, `availability_overrides`, `availability_weekday_rules`, `delivery_zones`, `daily_stats`, `analytics_events`, `audit_logs`) with indexed lookup per outlet), **unique index rebuilds** (tenant-scoped uniqueness updated to include `outlet_id` where the business constraint is per-location — inventory, tables, closures, stats), **default "Main Branch" outlet seed** (every existing tenant gets one automatically, all legacy data backfilled), and **Sequelize model layer** (`Outlet`, `OutletMembership`, `OutletMenuOverride` with Tenant/User/Product associations) | **Migration 028 + drift/v1→v2 suites — 621 backend tests green** (53 files) · coverage gate green |
