...
| **Phase 3** — Multi-tenant SaaS | Tenant workspaces, team members & roles, plans/subscriptions/feature flags, tenant-scoping middleware (fail-closed isolation), CSRF protection, Dhaka seed data (20 workspaces, 89 menu items) — **hardened**: **plan quota enforcement** (products / orders-per-day / members / storage gates with live usage meters), **expiring team invites** (token links, 1–30 day expiry, revoke, accept-with-account flow), **ownership transfer**, **tenant audit-log UI** (who changed what), **platform-admin plan changes** (migration 017) | Tenant isolation + CSRF suites + **quota/invite/transfer/audit suites — 477 backend tests** · **27 e2e** · PG job green |
...
```bash
cd backend
npm test                      # Vitest — 477 tests across 47 suites (2 skipped locally)
